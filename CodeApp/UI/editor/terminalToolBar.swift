    ...
    var body: some View {
        HStack{
                ...
                if UIPasteboard.general.hasStrings {
                    Button(action: {
                        if let string = UIPasteboard.general.string?.base64Encoded() {
                            evaluateScript("editor.trigger('keyboard', 'type', {text: decodeURIComponent(escape(window.atob('\(string)')))})")
                        }
                    }, label: {
            ...
            Group{
                ...
                Button(action: {
                    evaluateScript("editor.setPosition({lineNumber: editor.getPosition().lineNumber - 1, column: editor.getPosition().column})")
                }, label: {
                ...
                Button(action: {
                    evaluateScript("editor.setPosition({lineNumber: editor.getPosition().lineNumber, column: editor.getPosition().column - 1})")
                }, label: {
