        ...
        HStack(spacing: horizontalSizeClass == .compact ? 8 : 14){
            ...
            Group{
                ForEach(["{","}","[","]","(",")"], id: \.self){ char in
                    Button(action: {
                        App.monacoInstance.executeJavascript(command: "editor.trigger('keyboard', 'type', {text: '\(char)'})")
                    }, label: {
                ...
                if horizontalSizeClass != .compact {
                    Button(action: {
                        App.monacoInstance.executeJavascript(command: "editor.setPosition({lineNumber: editor.getPosition().lineNumber - 1, column: editor.getPosition().column})")
                    }, label: {
                        Image(systemName: "arrow.up")
                    })
                    Button(action: {
                        App.monacoInstance.executeJavascript(command: "editor.setPosition({lineNumber: editor.getPosition().lineNumber + 1, column: editor.getPosition().column})")
                        App.monacoInstance.executeJavascript(command: "editor.trigger('', 'selectNextSuggestion')")
                    }, label: {
                ...
                Button(action: {
                    App.monacoInstance.executeJavascript(command: "editor.setPosition({lineNumber: editor.getPosition().lineNumber, column: editor.getPosition().column - 1})")
                }, label: {
