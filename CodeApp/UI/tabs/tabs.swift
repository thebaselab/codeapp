        ...
        ScrollView(.horizontal, showsIndicators: false){
            HStack(spacing: 0){
                ForEach(Array(App.editors.enumerated()), id: \.element) { i, currentEditor in
                    editorTab(currentEditor: currentEditor, isActive: (App.activeEditor == currentEditor), index: i
                    , onOpenEditor: {
                    ...
                    .onDrop(of: [UTType.text], delegate: DragRelocateDelegate(item: currentEditor, listData: $App.editors, current: $dragging))
                }
            }
        }
    }
