...
public struct HierarchyList<Data, RowContent>: View where Data: RandomAccessCollection, Data.Element: Identifiable, RowContent: View {
    ...
    public init(data: Data, children: KeyPath<Data.Element, Data?>, expandStates: Binding<[AnyHashable:Bool]>, rowContent: @escaping (Data.Element) -> RowContent, onDisclose: @escaping (AnyHashable) -> Void) {
        self.recursiveView = RecursiveView(data: data, children: children, rowContent: rowContent, expandStates: expandStates, onDisclose: onDisclose)
    }
    ...
    var body: some View {
        ForEach(data) { child in
            if let subChildren = child[keyPath: children] {
                FSDisclosureGroup(expandStates: $expandStates, id: child.id, content: {
                    RecursiveView(data: subChildren, children: children, rowContent: rowContent, expandStates: $expandStates, onDisclose: onDisclose)
                }, label: {
                    rowContent(child)
...
//    @State var isTargeted = false
    ...
    init(expandStates: Binding<[AnyHashable:Bool]>, id: AnyHashable, content: @escaping () -> Content, label: @escaping () -> Label, onDisclose: @escaping (AnyHashable) -> Void){
