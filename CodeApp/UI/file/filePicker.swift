        ...
        func documentPicker(
            ...
            func saveToTemp(string: String) {
                ...
                let newEditor = EditorInstance(
                    url: url.absoluteString, content: string, type: .file)
            ...
            if let data = try? Data(contentsOf: url) {
                ...
                do {
                    ...
                    parent.App.notificationManager.showInformationMessage(
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                        if let data = try? Data(contentsOf: url) {
                            if let string = String(data: data, encoding: .utf8) {
                                ...
                                self.parent.App.notificationManager.showErrorMessage(
                                    "File not supported")
                            }
                        }
                    }
