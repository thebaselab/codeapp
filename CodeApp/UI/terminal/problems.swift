                        ...
                        DisclosureGroup(isExpanded: $expanded){
                            ...
                            ForEach(Array(App.problems[url]!), id: \.id){ marker in
                                HStack(alignment: .top){
                                    ...
                                    case 1:
                                        Image(systemName: "exclamationmark.triangle").foregroundColor(.yellow) // hint
                                    case 2:
                                        Image(systemName: "info.circle").foregroundColor(.blue) // info
                                    case 4:
                                        Image(systemName: "exclamationmark.triangle").foregroundColor(.yellow) // warning
                                    default:
                                        Image(systemName: "xmark.circle").foregroundColor(.red)
                                    }
                                    
                                    Text("\(marker.message)").foregroundColor(Color.init("T1")) +
                                        Text(" [\(marker.startLineNumber), \(marker.startColumn)]").foregroundColor(Color.gray)
...
//                                Text("monaco-tm").foregroundColor(.gray)
                                Circle()
                                    ...
                                    .overlay(
                                        Text("\(App.problems[url]!.count)").foregroundColor(Color.init("T1")).font(.system(size: 10))
                                    )
                            }
