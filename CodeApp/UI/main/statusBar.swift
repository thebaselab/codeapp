...
//                                    }
                ...
                HStack{
                    Group{
                        ...
                        if #available(iOS 15, *) {
                            Button("Select All"){
                                App.monacoInstance.executeJavascript(command: "if(document.activeElement.className.includes('monaco')){editor.setSelection(editor.getModel().getFullModelRange())}else if(document.activeElement.className=='input'){document.execCommand('selectAll')};")
