                ...
                Section(header:
                    ...
                    ZStack(alignment: .leading){
                        if App.commitMessage.isEmpty {
                            Text("Message (⌘Enter to commit)").font(.system(size: 14)).foregroundColor(.gray).padding(.leading, 3)
                        }
                    ...
                    HStack{
                        if App.indexedResources.isEmpty {
                            Text("There are no staged changes.").foregroundColor(.gray).font(.system(size: 12, weight: .light))
                        }else{
                            Text("Commit \(App.indexedResources.count) file\(App.indexedResources.count > 1 ? "s" : "") on '\(App.branch)'...").foregroundColor(.gray).font(.system(size: 12, weight: .light))
                        }
                        ...
                        Button(action: {
                            if let isSigned = App.gitServiceProvider?.isSigned(), !isSigned{
                                ...
                                App.notificationManager.showWarningMessage("There are no staged changes")
                            }else if App.commitMessage.isEmpty {
                                App.notificationManager.showWarningMessage("Commit message cannot be empty")
                            }else {
                                App.gitServiceProvider?.commit(message: App.commitMessage, error: {
                                    ...
                                    App.notificationManager.showInformationMessage("Commit succeeded")
                                }
                            }
                        }){
...
//                                }
                                Button(action: {
                                    App.notificationManager.showInformationMessage("Pushing to remote")
...
//                                }
                                Button(action: {
                                    App.notificationManager.showInformationMessage("Fetching from origin")
                                    App.gitServiceProvider?.fetch(error: {
                                        App.notificationManager.showErrorMessage($0.localizedDescription)
                                    }){
                                        App.notificationManager.showInformationMessage("Fetch succeeded")
                                        App.git_status()
                                    }
                                }){
                            ...
                            Section {
                                Button(action: {
                                    let path = App.workingResources.keys.map{$0.absoluteString.removingPercentEncoding!}
...
//                            }
                ...
                Section(header:
                    ...
                    if App.workingResources.isEmpty{
                        Text("No changes are made in the working directory.").foregroundColor(.gray).font(.system(size: 12, weight: .light))
                    }
...
//                }
                ...
                Section(header:
                    ...
                    Text("The folder currently opened doesn't have a git repository.").foregroundColor(.gray).font(.system(size: 12, weight: .light)).lineLimit(2)
                    
                    HStack(){
                        Spacer()
                        Text("Initialize Repository").foregroundColor(.white).font(.system(size: 14, weight: .light)).lineLimit(1)
                        Spacer()
                    }.onTapGesture{
                        if username.isEmpty || !email.contains("@"){
                            ...
                            App.gitServiceProvider?.initialize(error: { err in
                                DispatchQueue.main.async {
                                    App.notificationManager.showErrorMessage(err.localizedDescription)
                                }
                            }){
                                DispatchQueue.main.async {
                                    App.notificationManager.showInformationMessage("Repository initialized")
                                }
                                App.git_status()
                            }
                        }
                ...
                Section(header:
                    ...
                    ForEach(App.searchManager.searchResultItems, id: \.html_url){item in
                        VStack(alignment: .leading){
                            HStack{
                                RemoteImage(url: item.owner.avatar_url).frame(width: 20, height: 20).cornerRadius(5)
                                Text(item.owner.login).font(.system(size: 12)).foregroundColor(Color.init("T1"))
                            }
                            
                            Text(item.name).font(.system(size: 14)).fontWeight(.semibold).foregroundColor(Color.init("T1"))
                            if item.description != nil{
                                Text(item.description!).font(.system(size: 14)).foregroundColor(Color.init("T1"))
                            }
                            HStack{
                                Image(systemName: "star").font(.system(size: 12)).foregroundColor(.gray)
                                Text("\(item.stargazers_count)").font(.system(size: 12)).foregroundColor(.gray)
                                if item.language != nil{
                                    Text("\(item.language ?? "")  • \(humanReadableByteCount(bytes: item.size*1024))").font(.system(size: 12)).foregroundColor(.gray)
                                }else{
                                    Text("\(humanReadableByteCount(bytes: item.size*1024))").font(.system(size: 12)).foregroundColor(.gray)
                                }
                                Spacer()
                                Text("Clone").foregroundColor(.white).lineLimit(1).font(.system(size: 12)).padding(.leading, 8).padding(.trailing, 8).padding(.top, 4).padding(.bottom, 4).background(Color.init(accentColor)).cornerRadius(10).onTapGesture {
                                    
                                    do {
                                        let repo = item.name
                                        guard let dirURL = URL(string: App.workSpaceStorage.currentDirectory.url)?.appendingPathComponent(repo, isDirectory: true) else{
                                            return
                                        }
                    ...
                    if App.searchManager.errorMessage != "" {
                        HStack{
                            ...
                            Text(App.searchManager.errorMessage).font(.system(size: 12, weight: .light))
                    ...
                    HStack{
                        Image(systemName: "link")
                            .foregroundColor(.gray)
                            .font(.system(size: 14))
                        TextField("URL (HTTPS)", text: $gitURL, onCommit: {
                            do {
...
//                                }
                    ...
                    Text("Example: https://github.com/thebaselab/codeapp.git").foregroundColor(.gray).font(.system(size: 12, weight: .light)).lineLimit(2)
