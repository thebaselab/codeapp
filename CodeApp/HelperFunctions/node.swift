...
typealias RequestCompletionBlock = @convention(block) (_ uuid: UUID?, _ extensionItems: [Any]?) -> Void
...
func nodeCmd(args: [String]?) -> Int32{
    ...
    ext.setRequestInterruptionBlock({ uuid in
        if let uuid = uuid {
            ...
            nc.post(name: Notification.Name("node.stdout"), object: nil, userInfo: ["content": "Program interrupted. This could be caused by a memory limit or an error in your code.\n"])
    ...
    ext.setRequestCompletionBlock({ uuid, extensionItems in
        if let uuid = uuid {
            print("Request \(uuid) completed. Extension items: \(String(describing: extensionItems))")
        }
        
        if let item = extensionItems?.first as? NSExtensionItem {
            if let data = item.userInfo?["result"] as? String  {
                let nc = NotificationCenter.default
                nc.post(name: Notification.Name("node.stdout"), object: nil, userInfo: ["content": data])
            }
        }
    ...
    ext.beginExtensionRequestWithInputItems([item], completion: { uuid in
...
public func node(argc: Int32, argv: UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?) -> Int32 {
    ...
    if args?.count == 1 {
        fputs("Welcome to Node.js v16.13.2. \nREPL is unavailable in Code App.\n", thread_stderr)
        return 1
    }
...
public func npm(argc: Int32, argv: UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?) -> Int32 {
    ...
    if args.first == "run" {
        ...
        do{
            let data = try Data(contentsOf: URL(fileURLWithPath: workingPath), options: .mappedIfSafe)
            let jsonResult = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves)
            if let jsonResult = jsonResult as? Dictionary<String, AnyObject>, let scripts = jsonResult["scripts"] as? [String: String] {
                if var script = scripts[args[1]] {
                    ...
                    if let paths = try? FileManager.default.contentsOfDirectory(atPath: nodeBinPath) {
                        ...
                        for i in paths {
                            ...
                            if cmd == binCmd {
                                let moduleURL = sharedURL().appendingPathComponent("lib/bin/\(cmd)").resolvingSymlinksInPath()
                                
                                let prettierPath = moduleURL.path
                                
                                if var content = try? String(contentsOf: moduleURL) {
                                    if !content.contains("process.exit = () => {}"){
                                        content = content.replacingOccurrences(of: "#!/usr/bin/env node", with: "#!/usr/bin/env node\nprocess.exit = () => {}")
                    ...
                    guard FileManager.default.fileExists(atPath: bin.path) else {
                        fputs("npm ERR! command doesn't exist or is not supported: \(scripts[args[1]]!)\n", thread_stderr)
                        return 1
                    }
...
//    print(bin.path)
    ...
    guard FileManager.default.fileExists(atPath: bin.path) else {
        fputs("npm ERR! command doesn't exist or is not supported: \(args.joined(separator: " "))\n", thread_stderr)
        return 1
    }
...
public func nodeg(argc: Int32, argv: UnsafeMutablePointer<UnsafeMutablePointer<Int8>?>?) -> Int32 {
    ...
    let moduleURL = sharedURL().appendingPathComponent("lib/bin/\(args.removeFirst())").resolvingSymlinksInPath()
